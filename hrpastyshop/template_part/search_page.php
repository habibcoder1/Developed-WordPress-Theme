<?php if ( have_posts() ) : ?>
    ...
    <div class="search-result">
        <h2 class="search-title">
            <?php printf( __( 'Search Results for: %s', 'hrpasty'), '<span>' . get_search_query() . '</span>' ); ?>
        </h2>
    </div>
    <!-- Content Loop -->
            ...
            <div class="category"> 
                ...
                if (!empty($taxono)) {
                    ...
                    foreach ($taxono as $tax) {
                        ...
                        //Add comme between more taxonomy
                        echo ($i < count($taxono)) ? ", " : "";
                        $i++;
                    };
...
<?php else : ?>
    <!-- No Search Result -->
    <div class="search-result">
        <h2><?php printf( __( 'Sorry, No Result Found For: %s', 'hrpasty'), '<span>' . get_search_query() . '</span>' ); ?></h2>
    </div>
